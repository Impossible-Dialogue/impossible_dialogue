...
parser.add_argument(
    '-v', '--volume', type=float, default=0.1,
...
async def outputstream_generator_stereo(q_in1, q_in2, device_index):
    """Generator that streams blocks of audio data to a PyAudio output stream."""
    
    def callback(in_data, frame_count, time_info, status):
        assert frame_count == args.blocksize
        try:
            data1 = q_in1.get_nowait()
            data2 = q_in2.get_nowait()
            data = np.ndarray((args.blocksize, args.channels), dtype='float32')
            data[:,0] = data1[:]
            data[:,1] = data2[:]
        except asyncio.QueueEmpty:
            print('Buffer is empty: increase buffersize?', file=sys.stderr)
...
async def main(** kwargs):
    ...
    q_in5 = asyncio.Queue(maxsize=args.buffersize)
    q_in6 = asyncio.Queue(maxsize=args.buffersize)
    q_mix1 = asyncio.Queue(maxsize=args.buffersize)
    q_mix2 = asyncio.Queue(maxsize=args.buffersize)
    

    tasks = [
        asyncio.create_task(inputstream_generator(q_in1, 'media/test_heads/000_head_1_3d35ee80.wav')),
        asyncio.create_task(inputstream_generator(q_in2, 'media/test_heads/001_head_2_78eb6076.wav')),
        asyncio.create_task(inputstream_generator(q_in3, 'media/test_heads/002_head_3_bcc8f053.wav')),
        asyncio.create_task(inputstream_generator(q_in4, 'media/test_heads/003_head_4_0d18544c.wav')),
        asyncio.create_task(inputstream_generator(q_in5, 'media/test_heads/004_head_5_9edaa291.wav')),
        asyncio.create_task(inputstream_generator(q_in6, 'media/test_heads/005_head_6_d90787cf.wav')),
        asyncio.create_task(outputstream_generator_stereo(q_in1, q_in2, 0)),
        asyncio.create_task(outputstream_generator_stereo(q_in3, q_in4, 1)),
        asyncio.create_task(outputstream_generator_stereo(q_in5, q_in6, 2))
        # asyncio.create_task(streammixer_generator(q_mix1, [q_in1, q_in2])),
        # asyncio.create_task(streammixer_generator(q_mix2, [q_in3, q_in4])),
        # asyncio.create_task(outputstream_generator(q_mix1, 1)),
